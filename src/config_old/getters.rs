...
impl Config {
    ...
    // pub fn get_secret_envs(&self, password: impl AsRef<str>) -> HashMap<String, String> {
    //     let mut secret_envs: HashMap<String, String> = HashMap::new();
    //
    ...
    //         match parsed {
    //             Value::Table(t) => {
    //                 if let Some(env) = t.get("parameters") {
    //                     match env {
    //                         Value::String(s) => {
    //                             secret_envs.insert(k, s.to_string());
    //                         }
    //                         x => panic!("value for {k} is not a string, found {:?}", x),
    //                     }
    //                 }
    ...
    //                 //         Value::Table(t) => t.into_iter().for_each(|(k, v)| match v {
    //                 //             Value::String(s) => {
    //                 //                 secret_envs.insert(k, s);
    //                 //             }
    //                 //             x => panic!("value for {k} is not a string, found {:?}", x),
    //                 //         }),
    //                 //         x => panic!("value for {k} is not a table, found {:?}", x),
    //                 //     });
    //             }
    ...
    fn resolve_expression_values(&self, envs: &mut HashMap<String, (String, PathBuf)>) {
        ...
        for (key, (val, config_path)) in envs.clone().into_iter() {
            if val.starts_with("{{") && val.ends_with("}}") {
                ...
                let val =
                    read_with_dir_and_env(exp, workdir, &cleaned_envs).expect("valid expression");
    ...
    pub(crate) fn get_from_file(
        ...
        let val = self.config_file_map.get(&filepath)?.get(key)?;
    ...
    pub(crate) fn get_from_file_mut(
        ...
        let val = self.config_file_map.get_mut(&filepath)?.get_mut(key)?;
...
mod tests {
    ...
    fn test_get_from_file() {
        let path = PathBuf::from("tests/fixtures/config/nested/config.toml");
        ...
        let v = config.get_from_file("envs", &path).unwrap();
        assert!(matches!(v, toml::Value::Table(_)));

        let v = config
            .get_from_file("envs.AWS_PROFILE", &path)
            .unwrap()
            .as_str()
            .unwrap();
        assert_eq!(v, "default");
