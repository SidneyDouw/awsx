use crate::config2::tomlfile::traits::TomlFileInit;
use crate::config2::{verify_path::VerifiedPath, Config, ConfigFile};
use std::{
    collections::HashMap,
    fs::metadata,
    path::{Path, PathBuf},
...
pub enum Error {
    ...
    VerifyPath(#[from] crate::config2::verify_path::Error),
    ...
    ConfigFile(#[from] crate::config2::tomlfile::Error),
}
...
impl Config {
    pub fn from_path(path: &Path) -> Result<Self, Error> {
        let path = VerifiedPath::try_from(path)?;
